...
class NaturalLanguageClassifierV1(WatsonDeveloperCloudService):
    """Client for the Natural Language Classifier service."""
    ...
    def classify(self, classifier_id, text):
        ...
        :rtype: natural_language_classifier_v1.Classification
        """
        ...
        response = self.request(
            ...
            data=data,
            accept_json=True)
        return Classification._from_dict(response)

    def classify_get(self, classifier_id, text):
        """
        Classify (GET).

        The status must be `Available` before you can use the classifier to classify
        calls. Use `GET /classifiers/{classifier_id}` to retrieve the status.

        :param str classifier_id: Classifier ID to use.
        :param str text: Phrase to classify.
        :return: A `Classification` object
        :rtype: natural_language_classifier_v1.Classification
        """
        if classifier_id is None:
    ...
    def create_classifier(self,
        ...
        :rtype: natural_language_classifier_v1.Classifier
        """
        if metadata is None:
            raise WatsonInvalidArgument('metadata must be provided')
        if training_data is None:
            raise WatsonInvalidArgument('training_data must be provided')
    ...
    def list_classifiers(self):
        ...
        :return: A `ClassifierList` object
        :rtype: natural_language_classifier_v1.ClassifierList
        """
        response = self.request(
            method='GET', url='/v1/classifiers', accept_json=True)
        return ClassifierList._from_dict(response)
