...
def test_dialog_nodes():
    ...
    conversation.create_dialog_node('id', 'location-done', user_label='xxx')
...
def test_delete_user_data():
    ...
    response = conversation.delete_user_data('id').get_result()
    assert response is None
    assert len(responses.calls) == 1

@responses.activate
def test_list_mentions():
    url = 'https://gateway.watsonplatform.net/conversation/api/v1/workspaces/workspace_id/entities/entity1/mentions'
    responses.add(
        responses.GET,
        url,
        body='[{"entity": "xxx"}]',
    ...
    response = conversation.list_mentions('workspace_id', 'entity1').get_result()
