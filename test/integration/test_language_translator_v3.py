...
class TestIntegrationLanguageTranslatorV3(unittest.TestCase):
    ...
    def setup_class(cls):
        ...
        cls.language_translator.set_default_headers({
            'X-Watson-Test':
            '1'
        })

    def test_translate(self):
        translation = self.language_translator.translate(text='Hello', model_id='en-es').get_result()
    ...
    def test_document_translation(self):
        ...
        document_status = self.language_translator.get_document_status(document_id).get_result()
        assert document_status is not None

        if document_status.get('status') == 'available':
            response = self.language_translator.get_translated_document(document_id, 'text/plain').get_result()
            assert response.content is not None

        list_documents = self.language_translator.list_documents().get_result()
        assert list_documents is not None

        delete_document = self.language_translator.delete_document(document_id).get_result()
